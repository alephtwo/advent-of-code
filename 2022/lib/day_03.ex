  ...
  * Uppercase item types `A` through Z have priorities `27` through `52`.
  ...
  def part_one(input) do
    ...
    |> Enum.flat_map(fn {first, second} ->
      one = first |> String.graphemes() |> MapSet.new()
      two = second |> String.graphemes() |> MapSet.new()
  ...
  def part_two(input) do
    input
    |> parse_input()
    # break out a group for every three
    |> Enum.chunk_every(3)
    |> Enum.flat_map(&process_trio/1)
    |> Enum.map(&determine_priority/1)
    |> Enum.sum()
  end

  defp determine_priority(<<ascii::utf8>>) do
    cond do
      # lowercase letters
      ascii >= 97 && ascii <= 122 ->
        ascii - 96
  ...
  defp process_trio(rucksacks) when length(rucksacks) == 3 do
    rucksacks
